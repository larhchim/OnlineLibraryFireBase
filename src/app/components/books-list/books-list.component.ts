...
export class BooksListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['position', 'reference', 'author', 'description', 'publishDate', 'title', 'action'];
  dataSource: any;
    ...
    this.bookService.getAll().subscribe(
      (res) => {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          this.dataSource = new MatTableDataSource<BookElement>(this.ELEMENT_DATA);
          ...
          this.openSnackBar("List of Books has been loaded Successfully", "dismiss");
        }
      },
      (err) => {
        throw new Error(err);
